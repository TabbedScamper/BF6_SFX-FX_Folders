...
# --- FX/SFX folder view (portal turbo patch) ---
var _hp_folder: Array = []   # active folder's source collections; empty = root
...
func _ready() -> void:
	...
	var margin_left = -1 * int(get_theme_stylebox(&"BottomPanel", &"EditorStyles").get_margin(SIDE_LEFT))
	self.add_theme_constant_override(&"margin_left", margin_left)
	var margin_right = -1 * int(get_theme_stylebox(&"BottomPanel", &"EditorStyles").get_margin(SIDE_RIGHT))
	self.add_theme_constant_override(&"margin_right", margin_right)
	var margin_top = -1 * int(get_theme_stylebox(&"BottomPanel", &"EditorStyles").get_margin(SIDE_TOP))
	self.add_theme_constant_override(&"margin_top", margin_top)
	...
	_render_semaphore = Semaphore.new()
	_thread = Thread.new()
	_thread.start(_thumbnail_thread)
...
func _queue_update_thumbnail(path: String) -> void:
	var id = get_path_hash(path)
	if not _thumbnails.has(id) or _queue_has_path(path):
		return

	_mutex.lock()
	var queue_item: Dictionary[StringName, Variant] = {&"path": path, &"thumb": _thumbnails[id]}
...
static func create_asset_no_thumbnail(id: int, path: String) -> Dictionary[StringName, Variant]:
	return {
		&"id": id,
		&"path": path,
		&"thumb": null,
...
func create_asset(path: String) -> void:
	...
	var id: int = get_path_hash(path)
	var new_asset: Dictionary[StringName, Variant] = _create_asset(id, path)
...
func update_tabs() -> void:
	...
	var _hp_vis: Array = []
	for c in _curr_lib:
		if not _hp_hidden(String(c.name)):
			_hp_vis.append(c)
	if _hp_vis.size():
		_collec_tab_bar.set_tab_count(_hp_vis.size())

		var popup: PopupMenu = _all_tabs_list.get_popup()
		popup.set_item_count(_hp_vis.size())

		for i: int in _hp_vis.size():
			_collec_tab_bar.set_tab_title(i, _hp_vis[i].name)
			_collec_tab_bar.set_tab_disabled(i, false)
			_collec_tab_bar.set_tab_metadata(i, _hp_vis[i])

			popup.set_item_text(i, _hp_vis[i].name)
	else:
...
func update_item_list() -> void:
	var assets: Array = []
	var folders: Array = []
	if not _hp_folder.is_empty():
		folders.append({"__hp_back": true})
		for c in _hp_folder:
			assets.append_array(c.assets)
	else:
		assets = _curr_collec.assets
		var cname := String(_curr_collec.name)
		if not _hp_hidden(cname) and cname != "[null]":
			var fx_cols: Array = []
			var mfx: Dictionary = _hp_find(cname + " FX")
			if not mfx.is_empty(): fx_cols.append(mfx)
			var gfx: Dictionary = _hp_find("FX - All Maps")
			if not gfx.is_empty(): fx_cols.append(gfx)
			if not fx_cols.is_empty():
				folders.append({"__hp_folder": fx_cols, "__hp_label": "FX"})
			var sfx_cols: Array = []
			var msx: Dictionary = _hp_find(cname + " SFX")
			if not msx.is_empty(): sfx_cols.append(msx)
			var gsx: Dictionary = _hp_find("SFX - All Maps")
...
static func _serialize_asset(asset: Dictionary[StringName, Variant]) -> Dictionary:
	return {"id": asset.id, "path": asset.path}


static func _serialize_assets(assets: Array[Dictionary]) -> Array[Dictionary]:
...
static func _serialize_collection(collection: Dictionary[StringName, Variant]) -> Dictionary:
	return {
...
static func _serialize_library(library: Array[Dictionary]) -> Array[Dictionary]:
	var serialized: Array[Dictionary] = []
...
static func _cfg_save_library(library: Array[Dictionary], path: String) -> void:
...
static func _json_save_library(library: Array[Dictionary], path: String) -> void:
	var serialized: Array[Dictionary] = _serialize_library(library)
...
func _get_thumb_cache_path(id: int) -> String:
	return _get_thumb_cache_dir().path_join(str(id)) + ".png"


func _save_thumb_to_disk(id: int, image: Image) -> void:
	var error: Error
	if not DirAccess.dir_exists_absolute(_get_thumb_cache_dir()):
		error = DirAccess.make_dir_absolute(_get_thumb_cache_dir())
		assert(error == OK, error_string(error))

	error = image.save_png(_get_thumb_cache_path(id))
	assert(error == OK, error_string(error))
...
func _thumbnail_thread() -> void:
	...
	while true:
		...
		call_deferred_thread_group(&"_render_thumb", item, _render_semaphore.post)
...
func _on_collection_tab_close_pressed(tab: int) -> void:
	var _hp_idx: int = _curr_lib.find(_collec_tab_bar.get_tab_metadata(tab))
	if _hp_idx >= 0:
		show_remove_collection_dialog(_hp_idx)
...
func _on_item_list_item_clicked(index: int, at_position: Vector2, mouse_button_index: int) -> void:
	if mouse_button_index != MOUSE_BUTTON_RIGHT:
		return
	var _hp_meta: Dictionary = _item_list.get_item_metadata(index)
	if _hp_meta.has("__hp_folder") or _hp_meta.has("__hp_back"):
		return
	...
	popup.connect(&"id_pressed", func(option: AssetContextMenu) -> void:
		...
		match option:
			...
			AssetContextMenu.COPY_ID:
				DisplayServer.clipboard_set(str(asset.id))
			...
			AssetContextMenu.REFRESH:
				for i: int in selected_assets:
					...
					_queue_update_thumbnail(asset.path)
		)
	self.add_child(popup)
	...
	if selected_assets.size() == 1:  # If only one asset is selected.
		...
		popup.add_item("Copy ID", AssetContextMenu.COPY_ID)
...
func _on_item_list_item_activated(index: int) -> void:
	...
	if asset.has("__hp_folder"):
		_hp_folder = asset["__hp_folder"]
		_item_list.deselect_all()
		update_item_list()
		return
	if asset.has("__hp_back"):
		_hp_folder = []
		_item_list.deselect_all()
		update_item_list()
		return
...
static func get_path_hash(path: String) -> int:
	return hash(path.to_lower())
...
class AssetItemList:
	...
	func _get_drag_data(at_position: Vector2) -> Variant:
		...
		for i: int in get_selected_items():
			...
			if asset.has("__hp_folder") or asset.has("__hp_back"):
				continue
	...
	func _make_custom_tooltip(_for_text: String) -> Object:
		...
		if asset.is_empty() or asset.has("__hp_folder") or asset.has("__hp_back"):
			return null
